...
static void LV_ATTRIBUTE_FAST_MEM argb4444_image_blend(lv_draw_sw_blend_image_dsc_t * dsc)
{
    ...
    if(dsc->blend_mode == LV_BLEND_MODE_NORMAL) {
        if(mask_buf == NULL && opa >= LV_OPA_MAX) {
            for(y = 0; y < h; y++) {
                for(x = 0; x < w; x++) {
                    lv_color32_t src_c32 = lv_argb4444_to_color32(src_buf_u16[x]);
                    lv_color_8_8_mix(lv_color32_luminance(src_c32), &dest_buf_l8[x], src_c32.alpha);
                }
                dest_buf_l8 = drawbuf_next_row(dest_buf_l8, dest_stride);
                src_buf_u16 = drawbuf_next_row(src_buf_u16, src_stride);
            }
            ...
            for(y = 0; y < h; y++) {
                for(x = 0; x < w; x++) {
                    lv_color32_t src_c32 = lv_argb4444_to_color32(src_buf_u16[x]);
                    lv_color_8_8_mix(lv_color32_luminance(src_c32), &dest_buf_l8[x], LV_OPA_MIX2(src_c32.alpha, opa));
                }
                dest_buf_l8 = drawbuf_next_row(dest_buf_l8, dest_stride);
                src_buf_u16 = drawbuf_next_row(src_buf_u16, src_stride);
            }
            ...
            for(y = 0; y < h; y++) {
                for(x = 0; x < w; x++) {
                    lv_color32_t src_c32 = lv_argb4444_to_color32(src_buf_u16[x]);
                    lv_color_8_8_mix(lv_color32_luminance(src_c32), &dest_buf_l8[x], LV_OPA_MIX2(src_c32.alpha, mask_buf[x]));
                }
                dest_buf_l8 = drawbuf_next_row(dest_buf_l8, dest_stride);
                src_buf_u16 = drawbuf_next_row(src_buf_u16, src_stride);
                mask_buf += mask_stride;
            ...
            for(y = 0; y < h; y++) {
                for(x = 0; x < w; x++) {
                    lv_color32_t src_c32 = lv_argb4444_to_color32(src_buf_u16[x]);
                    lv_color_8_8_mix(lv_color32_luminance(src_c32), &dest_buf_l8[x], LV_OPA_MIX3(src_c32.alpha, opa,
                                                                                                mask_buf[x]));
                }
                dest_buf_l8 = drawbuf_next_row(dest_buf_l8, dest_stride);
                src_buf_u16 = drawbuf_next_row(src_buf_u16, src_stride);
        ...
        for(y = 0; y < h; y++) {
            for(x = 0; x < w; x++) {
                lv_color32_t color_argb = lv_argb4444_to_color32(src_buf_u16[x]);
                if(mask_buf == NULL) color_argb.alpha = LV_OPA_MIX2(color_argb.alpha, opa);
                else color_argb.alpha = LV_OPA_MIX3(color_argb.alpha, mask_buf[x], opa);
                blend_non_normal_pixel(&dest_buf_l8[x], color_argb, dsc->blend_mode);
            }
