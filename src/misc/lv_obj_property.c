...
typedef void (*lv_property_set_int_t)(const struct _lv_obj_t *, int32_t);
typedef void (*lv_property_set_pointer_t)(const struct _lv_obj_t *, const void *);
typedef void (*lv_property_set_color_t)(const struct _lv_obj_t *, lv_color32_t);
typedef void (*lv_property_setter_t)(const struct _lv_obj_t *, lv_prop_id_t, const lv_property_t *);
...
typedef lv_color32_t (*lv_property_get_color_t)(const struct _lv_obj_t *);
...
static lv_result_t obj_property(const lv_obj_t * obj, lv_prop_id_t id, lv_property_t * value, bool set);
...
lv_result_t lv_obj_property_set(const lv_obj_t * obj, const lv_property_t * value)
{
    return obj_property(obj, value->id, (lv_property_t *)value, true);
}

lv_property_t lv_obj_property_get(const lv_obj_t * obj, lv_prop_id_t id)
{
    lv_result_t result;
    lv_property_t value;
    ...
    for(clz = obj->class_p ; clz; clz = clz->base_class) {
        ...
        for(uint32_t i = 0; i < clz->properties_count; i++) {
            ...
            if(prop->id == LV_PROPERTY_ID_ANY) {
                value->id = prop->id;
                if(set) return ((lv_property_setter_t)prop->setter)(obj, id, value), LV_RESULT_OK;
